...
from VISA_wrapper import *


NR1 = TestValueFromType(int)
NR2 = TestValueFromType(float)
NR2.to_string = types.MethodType( lambda self,value:'%f'%value, NR2)
NR3 = TestValueFromType(float)
# bin ???

QString = TestValueFromType(str)
QString.to_string = types.MethodType( lambda self,value:'"%s"'%value, QString)

class DPO3000(visa.Instrument):
    pass

#Acquisition Command Group 
_add_generic_get_command(DPO3000, 'ACQuire')
_add_generic_get_command(DPO3000,'ACQuire:MAXSamplerate')
_add_generic_get_set_command(
...
should be in %s"%(str(name), ' '.join(channel_list)))
	...
	if isinstance(value, str) or isinstance(value, unicode):
		...
		value = str(value) is value<100 else '1.00E+06'
	else:
	    raise TektronixScopeError("Impedance is %s. It should be in %s"%liste_string)
